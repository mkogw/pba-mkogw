...
void solve_laplace_gauss_sidel_on_grid(
  ...
  for (unsigned int ix = 0; ix < grid_size; ++ix) {
    for (unsigned int iy = 0; iy < grid_size; ++iy) {
      ...
      float val_up = vtx2val[(iy + 1) * grid_size + ix]; float val_down = vtx2val[(iy - 1) * grid_size + ix];
      float val_right = vtx2val[iy * grid_size + (ix + 1)]; float val_left = vtx2val[iy * grid_size + (ix - 1)];
      vtx2val[idx_center] = (val_up + val_down + val_right + val_left)/4.0;
    }
  }
}
